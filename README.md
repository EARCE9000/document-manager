...
LLMなどで作ったWebページのモックアップ(ビルド済みの一式)をZIPで登録し、**別ウィンドウでそのまま動かして確認する**ための機能。文書とは**別のコレクション**で、DBの表もファイルの置き場も分かれている。詳細は[docs/mockup.md](docs/mockup.md)。

- **既定は無効**。文書管理とは目的が違い、信用できないHTML/JSを配信し、原本と展開後の両方でディスクを使うため、要る人だけが開ける形にしている。開け方は2つ:
  - 管理画面(右上の歯車)→「サーバー」タブ→**機能のOn/Off**。DBに保存されるため**再起動は不要**で、複数インスタンス構成でも全台に効く。登録済みのモックアップは無効にしても消えない
  - 環境変数 `MOCKUPS_ENABLED=true`。その環境の**既定値**で、管理画面で変更するとそちらが優先される
  - どちらで開いても`STORAGE_BACKEND=local`の構成でのみ使える(それ以外では有効にできない)
- 無効の間は、APIが503を返し、画面にメニューを出さず、**OpenAPI・AI向けガイドからも消える**(使えないAPIをAIに呼ばせないため。意味検索と同じ扱い)
...
- **機能のOn/Off**: モックアップ管理など、使う構成でだけ開ける機能の切り替え(「サーバー」タブ内)。DBに保存するため再起動不要・全インスタンスに効く。今の状態が環境変数の既定のままか、誰かが画面で変えた値かも表示する
